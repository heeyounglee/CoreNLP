...
import edu.stanford.nlp.hcoref.sieve.LinearSieve;
...
public class LinearSieveFactory {
...
//                      List<Mention> candidates = document.predictedOrderedMentionsBySentence.get(sentIdx-sentDist);
                    ...
                    for(Mention candidate : candidates) {
                      ...
                      RVFDatum<Boolean, String> datum = LinearSieve.extractDatum(m, candidate, document, mentionDist, cs.dictionaries, props, sievename);
...
//              traindataOneDoc.addAll(trainNegativeData.sampleDataset(0, options.downsamplingRate, false));
  ...
  // train a sieve
  public static LinearSieve boostTrain(RVFDataset<Boolean, String> traindata, String pathModel, Properties props, String sievename) throws Exception {
    ...
    LinearClassifier<Boolean, String> classifier = null;
    LinearSieve sieve = new LinearSieve(classifier, props, sievename);
    ...
    for(int iter=0 ; iter<=MAX_ITER ; iter++) {
      ...
      classifier = trainClassifier(downsampled, props, sievename);
      sieve = new LinearSieve(classifier, props, sievename);
  ...
  private static RVFDataset<Boolean, String> downsampledTraindata(RVFDataset<Boolean, String> traindata, LinearSieve sieve, Properties props) throws Exception {
    if(CorefProperties.getDownsamplingRate(props, sieve.sievename)==1) return traindata;   // no downsampling
    
    LinearClassifier<Boolean, String> classifier = sieve.classifier;
    ...
    // input: (traindataPerThread, classifier, instancesForAttrInfo, options), output: (positivesPerThread, negativeHardPerThread, negativeEasyPerThread)
        ...
        nThreads, new ThreadsafeProcessor<List<Object>, List<Object>>() {
          ...
          public List<Object> process(List<Object> input) {
            try {
              ...
              LinearClassifier<Boolean, String> classifier = (LinearClassifier<Boolean, String>) input.get(1);
              ...
              for(RVFDatum<Boolean, String> datum : traindataPerThread) {
                ...
                if(classifier==null) {
                  ...
                  double probTrue = classifier.probabilityOf(datum).getCount(true);
  ...
  public static LinearClassifier<Boolean, String> trainClassifier(RVFDataset<Boolean, String> traindata, Properties props, String sievename) throws Exception {
	  LinearClassifierFactory<Boolean, String> factory = new LinearClassifierFactory<Boolean, String>();
	  LinearClassifier<Boolean, String> classifier = factory.trainClassifier(traindata);
    return classifier;
  }
