...
public class LinearSieve extends Sieve {
  
  private static final long serialVersionUID = 1639567087069063446L;

/** the probability threshold for merging two mentions */
  public double thresMerge;
  
  public LinearClassifier<Boolean, String> classifier;
  
  // constructor for RF sieve
  public LinearSieve(LinearClassifier<Boolean, String> classifier, Properties props, String sievename) {
    super(props, sievename);
    this.props = props;
    this.classifierType = ClassifierType.LINEAR;
    this.classifier = classifier;
  }

  public boolean unlikelyCoreferentPair(Mention candidate, Mention m) {
    if(candidate.mentionType == MentionType.NOMINAL && m.mentionType == MentionType.PROPER) return true;
    if(candidate.mentionType == MentionType.LIST && m.mentionType != MentionType.LIST) return true;
    if(candidate.mentionType == MentionType.PRONOMINAL && m.mentionType != MentionType.PRONOMINAL) return true;
    return false;
  }
  ...
  public void findCoreferentAntecedent(Mention m, int mIdx, Document document, Dictionaries dict, Properties props, StringBuilder sbLog) throws Exception {
    ...
    for(int sentDist=0 ; sentDist <= Math.min(this.maxSentDist, sentIdx) ; sentDist++) {
      ...
      for(Mention candidate : candidates) {
        ...
        // TODO for singlepass experiment
//        if(unlikelyCoreferentPair(candidate, m)) continue;
        ...
        double probTrue = 0;
        if(this.classifierType == ClassifierType.LINEAR) {
        	probTrue = this.classifier.probabilityOf(datum).getCount(true);
        }
